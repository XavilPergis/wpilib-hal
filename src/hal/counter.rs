...
// FIXME
pub fn initialize_counter(mode: CounterMode, index: &mut i32) -> HalResult<CounterHandle> {
    hal_call![ ptr HAL_InitializeCounter(mode.into_raw(), index) ].map(|n| CounterHandle(n))
}
...
pub fn set_counter_pulse_length_mode(handle: CounterHandle, threshold: f64) -> HalResult<()> {
    hal_call![ ptr HAL_SetCounterPulseLengthMode(handle.get_handle(), threshold) ]
}
...
pub fn get_counter_stopped(handle: CounterHandle) -> HalResult<bool> {
    hal_call![ ptr HAL_GetCounterStopped(handle.get_handle()) ].map(|n| n != 0)
}
